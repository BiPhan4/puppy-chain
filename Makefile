...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=canine \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=puppyd \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/puppyd ./cmd/puppyd
endif

build_cli:
	go build -o build/puppyd -mod=readonly -tags "$(GO_TAGS) build/puppyd" -ldflags '$(LD_FLAGS)' ./cmd/puppyd
...
install: check-go-version go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/puppyd
...
draw-deps:
	...
	@goviz -i ./cmd/puppyd -d 2 | dot -Tpng -o dependency-graph.png
